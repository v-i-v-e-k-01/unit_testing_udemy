...
namespace TestNinja.Mocking
{
    public  class HousekeeperService
    {
        ...
        private readonly IStatementGenereator _statementGenereator;
        private readonly IXtraMessageBox _xtraMessageBox;
        public HousekeeperService(IUnitOfWork unitOfWork, IEmailSender emailSender, IStatementGenereator statementGenerator, IXtraMessageBox xtraMessageBox)
        {
            _unitOfWork = unitOfWork;
            _emailSender = emailSender;
            _statementGenereator = statementGenerator;  
            _xtraMessageBox = xtraMessageBox;
        }

        public void SendStatementEmails(DateTime statementDate)
        {
            var housekeepers = _unitOfWork.Query<Housekeeper>();  // Note that UnitOfWork class is declared in BookingHelper file
            ...
            foreach (var housekeeper in housekeepers)
            {
                ...
                var statementFilename = _statementGenereator.SaveStatement(housekeeper.Oid, housekeeper.FullName, statementDate);
